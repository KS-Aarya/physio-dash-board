...
export async function POST(request: NextRequest) {
	try {
		...
		// Check if we're in staging - disable SMS for staging, only allow in production
		const isStaging = process.env.NEXT_PUBLIC_ENVIRONMENT === 'staging';
		if (isStaging) {
			const message = generateSMSMessage(smsData.template, smsData.data);
			console.log('SMS would be sent (staging mode - SMS disabled):', {
				to: formattedPhone,
				message,
				template: smsData.template,
			});
			return NextResponse.json({ 
				success: true, 
				message: 'SMS logged (staging mode - SMS disabled)',
				preview: message,
			});
		}
		...
		// Send SMS via Twilio (only in production)
		try {
